...
- **Phase 1 — Increment A shipped** (`983ea00`): spec validator + ghost primitives + renderer + `?spec=demo` route.
- **Phase 1 — Increment B done**: eventbus + session store (`screen` model, localStorage, `ui_state` snapshot) + One-Thing wiring in `core/maya.js` (strict surface swap, follow-up re-target without remount, result→response bloom hand-off). Verified headless (see Increments below).
- **On-screen overlap batch done**: (1) legacy tools hold their reply instead of printing it in the reading dock over the interface (`maya.complete` → `_onLegacyComplete` hand-off); (2) `materializer.pickPosition` now clamps tool bottoms above the **sacred bottom band** (composer + footer) and clears a small **top-left anchor zone** — nothing can touch the footer or the anchor dot; (3) the top-left `.chat-collapse` dot is a **general-purpose anchor** that opens a compact **chat summary** panel (digest of `store.conversation`, never the whole thread), closed by outside-tap/Escape. Side-note: the "HERE over the calculator" overlap was exposed by the Increment B `has-tool` dim removal — the hand-off is the real fix.
- **Composer + send done**: the composer (`#maya-input`) rests at `bottom: calc(5% + 52px)` so it clears the footer by ≥10px on every viewport (measured 36–50px gap); the thinking whisper rose to 24% to clear the raised composer; a **send light** (`#maya-send`, ghost circle on the pill's right shoulder) appears when the composer rests and triggers the same genie as Enter (hidden during morph/capture, skips tab order, empty no-op). Verified headless at 3 viewports + click-to-send mounts a tool.
- **Backend history contract fixed (`535231a` next)**: `_askBackend` was shipping `history` entries as `{role, text, tool}`, but the backend `ChatMessage` requires `{role, content}` → every tier-2 turn after the first got **422** and fell back to *"I did not quite catch that."* Now the frontend maps entries to `{role, content}`. Verified: a two-entry history POST returns 200 with a sane reply; headless two-turn tier-2 conversation passes (`hello` → `hello again`), conversation grows, zero errors.
- **Increment C — backend session seam done (**kaushix-api**, commit after `a2598ad`)**:
  - `schemas.py`: `AssistantRequest` now accepts optional `session_id` + `ui_state` (pydantic extra keys were previously silently dropped).
  - New `sessions.py`: in-memory `sessions.Sessions` store — TTL 30 min, cap 1000 sessions, conversation capped at 40 turns, thread-safe. No Redis (honest minimum for a modular monolith).
  - `routes.py`: the seam is **opt-in** — requests carrying `session_id` get server-authoritative history (client tail is only a bootstrap), on-screen `ui_state` context, turn persistence, and the `session_id` echoed back; requests without `session_id` keep the exact baseline response shape and call path (verified: no regressions — the 9 pre-existing suite failures are unchanged before/after).
  - `services.py`: `build_messages`/`_complete`/`generate_chat_response` accept a `screen` context; `describe_screen()` turns `ui_state` into a compact `[on screen] screen task: …; components: …` system note so follow-ups like *"only the liability changes"* resolve against the live UI.
  - Verified: new tests pass (`test_maya_session_persists_followup`, `test_maya_session_bootstrap_uses_client_history`, `test_sessions_store_ttl_and_cap`, `test_describe_screen`); suite back to the pre-existing 9 failures only.
  - Remaining Increment C: `agents/intent.py` (anaphora → ui_spec), `ui_generator`, extended response (`ui_spec`/`intent` keys — frontend `_askBackend` already reads `data.ui_spec`/`data.intent`), SSE stub, `web.search`/`web.facts` tool seam.
- **Generative screen + send light done (frontend-only round)**:
  - **Send-light bug fixed**: the dark-tap capture handler's exclusion list (`maya.js` pointerdown) now includes `#maya-send` — previously a press on the send light with the composer up (`state="typing"`) folded the composer and cleared the input before the click read it, so the send never fired. Root cause proved live; fixed; now `click→capture→answer` works headless with the *real* typing-state click.
  - **One fixed generative screen at the true centre**: `materializer.pickPosition` randomness removed → `computeSlot()` single-centre slot (centred, clamped above the sacred bottom band and clear of the top-left anchor); the transcript echoed turns now live at the true centre; changeovers are slow crossfades (`maya-screen-unveil` 1.4s / `maya-screen-diffuse` 1.2s, shared by responses and `.maya-tool`s) with `_transitionPulse()` driving a `VoiceRenderer.surge()` particle breath + burst into the slot.
  - **Home greeting + on-screen thread killed**: `#maya-prompt` hides at the first interaction (`_sessionActive` in capture/handleText/showInterface) and never returns mid-session (all re-shows gated on `!_sessionStarted`); the growing top-left chat constellation (`_appendToChat`/`_flyInto`/`_settleThread`, `.chat-spine`/`.chat-entry*`) is gone — conversation lives only in `store.conversation` behind the top-left anchor summary; the anchor dot reveals itself on the first turn (`_revealAnchor`).
  - **Store mirrors the screen**: `_say`/`_sayResponse` now `store.update({reply})` so `ui_state` describes the live One-Thing when the ui_spec generator lands.
  - **Verified headless** (18/18 generative-screen + all prior suites green): send works through the real composer flow, banner hidden after first turn and never returns after Escape-folds, zero `.chat-entry`/spine DOM, two different experiences land in the **same slot** (same centre/`cx`, differing only by their own height), response centre = screen centre, tools never touch footer, zero JS errors. `?spec=demo` Increment B suite + composer + backend two-turn contract still green.
...
- **A — Spec core (frontend):** ~~validator + ghost primitives + renderer + `?spec=demo` route.~~ **Done** (shipped with `983ea00`): `ui/spec.js` validator + `ui/primitives.js` + renderer + `?spec=demo` route. *Verified: demo spec renders headless; 10/10 validator checks pass (unknown types, >4 components, grid layout, bad/dangling/dedup refs all rejected).*
- **B — Session + events + One-Thing:** ~~store (`screen` model), eventbus, action→store wiring, follow-up path + no-remount re-target, exclusivity swap (interface ≁ response), result→response hand-off, response center-bloom → rest-low.~~ **Done**: `events/eventbus.js` (`intent · ui:spec · ui:action · tool:run · tool:result · session:update`), `session/store.js` (observable `screen` persisted to localStorage, `snapshot()` → `ui_state`), `core/maya.js` One-Thing wiring: `_showInterface`/`_retargetInterface` (same-type re-target in place, no remount), `_sayResponse` (center bloom → rest-low `is-born`/`is-quiet`), strict surface swap (no `has-tool` dim), result→response hand-off, lead-in into thinking whisper; `_askBackend` now sends `session_id`+`ui_state` and parses the new `{intent, ui_spec, reply}` shape. *Verified headless: (1) `?spec=demo&auto=1` — interface mounts, compare runs, interface dissolves, "done — 4 lines removed, 5 added." becomes the one response; response and interface never co-visible (30ms poll); store surface settles to `response`. (2) follow-up spec re-targets the SAME DOM node — no remount, diff narrows in place ("2 removed · 2 added"), componentStates/results captured. (3) reload keeps `sessionId` + thread, restores no live interface. Legacy 7-tool verification still green, zero console/HTTP errors.*
...
- **E — Hardening & regression:** 7 legacy tools still materialize; reading-dock/composer zones respected (no overlap) — **components now done** (sacred bottom band + top-left anchor clearance in `materializer.js`; legacy reply hand-off); `node --check` all modules; readme "Creating a Tool" updated for spec-driven tools.
